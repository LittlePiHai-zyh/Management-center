...
public class NioWebSocketHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) throws Exception {
        ...
        NettyConfig.getChannelMap().put(ctx.channel().id().asShortText(), ctx.channel());

        // 将用户ID作为自定义属性加入到channel中，方便随时channel中获取用户ID
        AttributeKey<String> key = AttributeKey.valueOf("userId");
        ctx.channel().attr(key).setIfAbsent(ctx.channel().id().asShortText());
...
//        String userId = ctx.channel().attr(key).get();
        NettyConfig.getChannelMap().remove(ctx.channel().id().asShortText());
    }
